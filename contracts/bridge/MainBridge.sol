...
contract MainBridge is EIP712 {
    ...
    external
  {
    SwapRequest memory request = SwapRequest({
      ...
      _chainTo: _chainTo,
      tokenURI: Academy721(eth).tokenURI(_itemId),
      swapper: msg.sender,
      to: _to,
    ...
    Academy721(eth).transferFrom(msg.sender, address(this), _itemId);

    emit SwapInitialized(hash, eth, _itemId, msg.sender, _chainFrom, _chainTo, _to);
  }
  ...
  function redeem(bytes32 hash, uint8 v, bytes32 r, bytes32 s) external {
    ...
    redeemed[hash] = true;
    Academy721(eth).safeMint(address(this), "uri");
